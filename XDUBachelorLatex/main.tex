...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.17\linewidth}|p{0.21\linewidth}|p{0.25\linewidth}|p{0.25\linewidth}|}
        ...
        \hline
        匿名测试日志与反馈优化闭环 & 原测试部分容易停留在功能通过，缺少用户反馈和数据来源。 & 5.3 assistant\_test\_log 数据表；7.1--7.5 测试环境、日志说明和用户体验测试；附录 A.7。 & 7.5 用户体验测试与反馈分析；附录 A.8--A.10。 \\
        \hline
    \end{tabular}
\end{table}
...
本章对应前文提出的“可验证评价方式”和“测试与反馈闭环”创新点，重点回答系统实现效果是否能够被数据和测试过程支撑。7.1 说明测试环境、数据来源和能力边界；7.2 通过后端自动化测试验证核心服务分支；7.3 验证大模型意图解析和服务分流效果；7.4 通过功能测试、FAQ 检索统计和边界用例验证核心功能；7.5 通过匿名反馈测试日志分析用户体验；7.6 和 7.7 将测试结果回扣到交互优化、效率提升、能力边界和后续改进方向。
...
为弥补单纯人工体验测试样本量较小、数据来源不够稳定的问题，本文在后端 test 目录中补充了 JUnit 5 和 Mockito 自动化测试。测试不直接连接真实大模型服务和真实银行数据库，而是通过 Mock 对象构造固定返回值，从而重点验证业务分支、参数解析、知识库检索和日志记录逻辑。这样可以在每次代码修改后重复执行相同用例，避免只依赖一次性人工操作截图或主观反馈。
...
从表 \ref{tab:intent-recognition-test} 可以看出，在本次 80 条典型测试样本中，系统通过 76 条，综合通过率为 95.0\%。其中，balance、transactions、statistics 和 transfer 四类核心业务意图共 40 条，通过 37 条；FAQ 咨询和不支持能力样本在模型层均应先归入 other，再分别进入 faq\_knowledge\_base 或 qwen\_fallback；情绪安抚、多轮状态和确认取消则属于服务编排层分流测试，主要验证系统是否按照源码中的 handleEmotionSupport、handlePendingTransfer 和 handlePendingTransactionQuery 进入正确处理路径。未通过样例及原因如表 \ref{tab:intent-error-samples} 所示。
...
\begin{table}[H]
    ...
    \scriptsize
    \setlength{\tabcolsep}{2pt}
    \begin{tabular}{|p{0.06\linewidth}|p{0.14\linewidth}|p{0.07\linewidth}|p{0.07\linewidth}|p{0.07\linewidth}|p{0.07\linewidth}|p{0.07\linewidth}|p{0.07\linewidth}|p{0.07\linewidth}|}
        \hline
        编号 & 任务名称 & 记录数 & 完成率 & 响应耗时/s & 步数 & 理解度 & 满意度 & 安全感 \\
        \hline
        U1 & 查询账户余额 & 9 & 100.0\% & 2.72 & 2.3 & 4.7 & 4.3 & 4.7 \\
        \hline
        U2 & 查询支出流水 & 9 & 100.0\% & 2.66 & 3.3 & 4.0 & 3.7 & 4.0 \\
        \hline
        U3 & 银行卡安全 FAQ & 9 & 100.0\% & 2.56 & 3.0 & 4.2 & 4.0 & 5.0 \\
        \hline
        U4 & 转账到账 FAQ & 9 & 100.0\% & 2.61 & 3.0 & 4.0 & 4.0 & 4.0 \\
        \hline
        U5 & 语音查询余额 & 9 & 100.0\% & 2.74 & 3.9 & 3.7 & 3.7 & 3.7 \\
        \hline
        U6 & 支出统计图表 & 9 & 100.0\% & 6.90 & 3.3 & 3.8 & 3.7 & 4.0 \\
        \hline
        U7 & 转账后取消 & 18 & 100.0\% & 1.48 & 2.2 & 4.6 & 4.2 & 5.0 \\
        \hline
        U8 & 投诉情绪安抚 & 9 & 100.0\% & 0.18 & 3.0 & 4.1 & 4.0 & 5.0 \\
        \hline
    \end{tabular}
\end{table}

从表 \ref{tab:user-experience-summary} 可以看出，余额查询、银行卡安全 FAQ、转账取消和投诉情绪安抚的理解度和安全感评分较高，说明用户能够较快理解系统回复，并认可资金操作确认、取消回退和人工客服引导等安全设计。U5 语音查询余额的评分相对较低，主要反馈集中在浏览器权限弹窗、环境噪声和语音转写后仍需确认文本。U6 支出统计图表的平均响应耗时最高，且理解度、满意度低于余额查询和转账取消，说明统计口径、图表含义和文字总结仍需要进一步优化。U2 支出流水查询虽能完成，但普通用户反馈中出现“筛选词不太会说”“希望默认突出支出”等意见，说明自然语言提示和结果展示还可以继续简化。
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.06\linewidth}|p{0.06\linewidth}|p{0.32\linewidth}|p{0.44\linewidth}|}
        ...
        \hline
        P9 & U8 & 用户情绪激动时，请执行转人工分流并保留安抚语 & 情绪分流策略清晰，符合服务规范。 \\
        \hline
    \end{tabular}
\end{table}
