...
需要说明的是，该结果并不表示系统能够对任意开放式自然语言问题都保持 100\% 识别准确率。本次测试样本主要围绕系统已实现功能和答辩演示高频问法构造，样本构成见附录 \ref{app:test-samples}，目的是验证项目源码中已设计的意图解析提示词、JSON 输出约束和不支持能力拦截是否有效。测试样本由系统已实现功能边界内的典型问法构成，因此该结果不能外推为开放域自然语言理解准确率。对于更大规模真实用户语料，仍需要继续扩大样本数量，并引入准确率、召回率、混淆矩阵和人工复核等更完整的评估方法。
...
受测试条件和隐私保护要求限制，本文未采集真实银行用户的个人金融信息，而是基于上述用户画像设计 9 组匿名模拟测试记录，并使用系统内置虚拟账号完成任务走查。9 组记录中，P1--P3 对应计算机专业学生，P4--P6 对应非计算机专业人员，P7--P8 对应计算机行业开发者，P9 对应行业专家。该数据不是线上真实客户数据，而是原型系统在本地测试环境中的模拟用户体验数据，用于评估系统功能链路和交互设计是否具备基本可用性。数据来源与计算方式如表 \ref{tab:user-experience-source} 所示。

\begin{table}[H]
    \centering
    \caption{用户体验模拟测试数据来源说明}
    ...
    \begin{tabular}{|p{0.22\linewidth}|p{0.34\linewidth}|p{0.34\linewidth}|}
        ...
        \hline
        匿名模拟用户画像 & P1--P9 共 9 组记录，对应学生、普通用户、开发者和企业导师四类角色。 & 每个画像完成 U1--U8 共 8 项任务，形成 72 条任务记录。 \\
        \hline
...
基于上述记录得到的模拟测试结果如表 \ref{tab:user-experience-summary} 所示。

\begin{table}[H]
    \centering
    \caption{用户体验模拟测试结果汇总表}
...
从表 \ref{tab:user-experience-summary} 可以看出，8 项任务共形成 72 次模拟任务记录，其中 68 次完成，整体任务完成率为 94.4\%。余额查询、FAQ 问答、转账取消和投诉安抚任务完成情况较好，说明系统在高频咨询和安全确认场景中具有较清晰的交互路径。语音查询余额任务完成率相对较低，主要原因是浏览器语音识别权限、环境噪声和识别结果需要二次修改；支出统计图表任务平均耗时较长，说明用户需要一定时间理解统计口径和图表含义。整体来看，模拟测试结果能够反映系统具备基本可用性，但语音输入稳定性、统计结果解释和 FAQ 同义问法覆盖仍有继续优化空间。
